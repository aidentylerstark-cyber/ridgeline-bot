...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    // ── Ticket enhancements: priority, status, escalation, reopen, notes ──
    await client.query(`ALTER TABLE discord_tickets ADD COLUMN IF NOT EXISTS priority VARCHAR(10) NOT NULL DEFAULT 'normal'`);
    await client.query(`ALTER TABLE discord_tickets ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'open'`);
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS discord_ticket_notes (
        ...
        staff_discord_id VARCHAR(30) NOT NULL,
        content TEXT NOT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT NOW()
      )
    `);
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_discord_tickets_status
        ON discord_tickets (status) WHERE NOT is_closed
    `);
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS swipematch_profiles (
        id SERIAL PRIMARY KEY,
        discord_user_id VARCHAR(30) NOT NULL UNIQUE,
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS swipematch_swipes (
        id SERIAL PRIMARY KEY,
        swiper_id VARCHAR(30) NOT NULL,
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_swipematch_swipes_swiper
        ON swipematch_swipes (swiper_id)
    `);

    await client.query(`
      CREATE TABLE IF NOT EXISTS swipematch_matches (
        id SERIAL PRIMARY KEY,
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS swipematch_daily_limits (
        id SERIAL PRIMARY KEY,
        discord_user_id VARCHAR(30) NOT NULL,
    ...
    // SwipeMatch profile enhancements: rotating prompts + themes
    ...
    await client.query(`ALTER TABLE swipematch_profiles ADD COLUMN IF NOT EXISTS theme VARCHAR(30) NOT NULL DEFAULT 'default'`);
